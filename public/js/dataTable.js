/* global Page, Ara, DetailPage, Jui, USE_BS_UI, pdfMake, Thread, LIBS_URL, customRenderParams */
...
var DataTableCurrPage = 0;
var GridStartPage = 1;
var dtTextTR = {
	"sProcessing": "Yükleniyor...",
	"sLengthMenu": "<span class='hidden-xs'>Her sayfada</span> _MENU_ kayıt",
	"sZeroRecords": "Kayıt bulunamadı.",
	"sInfo": "Toplam _TOTAL_, gösterilen _START_ - _END_ <span class='hidden-xs'> arası kayıtlar</span>",
...
function JqDataTable(tableId, jSon)
{
	...
	DataTableCurrPage = dt.DataTable.CurrPage;
	DataTableRowButtons[tableId] = [];
	var customRender = dt.CustomRenderFunc;
	var isCustomRender = typeof window[customRender] == "function";
	var visibleLimit = isCustomRender ? 1000 : 30;
	var tbl = $('#' + tableId)
	...
	for(var i in dt.DataTable.Columns)
	{
		...
		if(c.GroupName == '')
			c.GroupName = 'Genel';
		var visible = c.Visible == 1 && (v++) < visibleLimit;
		var width = 'auto';
		if(c.Width)
			width = c.Width;
		cols.push({
	...
	var options = {
		...
		"bStateSave": true,
		"oLanguage": dtTextTR,
		"iDisplayStart" : (window['GridStartPage'] - 1) * parseInt(dt.PageSize),
			...
			if (DataTableAttr[tableId])
			{
				...
				for(var name in rowAttr)
					if (name.toLowerCase() == 'class')
						...
						$(nRow).attr(name, is_null(rowAttr[name], ''));
			}
			// Buttons
		...
		"fnDrawCallback": function (oSettings) {
			...
			if(trs.length > 0 && is_set(trs.first().attr('row_id')))
			{
				if(cb)
		...
		"fnServerData": function ( sSource, aoData, fnCallback ) {
			...
			if (typeof BlockUI == 'function')
				Page.ShowProcessingMessage($(this));
			else
				Page.Loading();
			...
			var vColumns = GetVisibleColumns(tableId);
			...
			if (customRenderParams != null)
			{
				...
				data = ProcessCustomRenderParams(data);
			}
            request = $.ajax({
                "dataType" : "json",
                "type" : "POST",
                ...
                "success" : function(e, x, a){
					if (dt.QueryParamsToUrl)
					{
						var url = GetUrl('#');
						var custom = {};
						if (typeof GetCustomParams == 'function')
							custom = GetCustomParams();
						var params = Ara(null, custom, 1);
						var settings = $('#' + e.tableId).dataTable().fnSettings();
						var sorgu = '';
						for(var name in params)
						{
							params[name] = params[name].replace(/[\/?]/g, ' ').replace(/\s+/g, ' ');
							if (name == 'sorgu')
								sorgu = params[name];
							if (! name.match(/(act[0-9]?|iDisplayStart)/))
							{
								if (UseRewriteAct == 1)
						...
						{
							if (UseRewriteAct == 1)
								url += '/' + settings._iDisplayStart;
							else
								url += '&iDisplayStart=' + settings._iDisplayStart;
						}
						if (window.StateBeingProcessed != 1)
						{
							if (! window.history.state ||
								window.history.state.sorgu != sorgu ||
								window.history.state.start != settings._iDisplayStart)
								window.history.pushState(
									{'sorgu' : sorgu, 'start' : settings._iDisplayStart},
									"", url.replace(/([^:])\/\//g, '$1/')
								);
						}
						var page = settings._iDisplayStart / settings._iDisplayLength + 1;
						var pageCount = Math.ceil(settings._iRecordsDisplay / settings._iDisplayLength);
						var pageStr = '';
						if (sorgu != '')
							pageStr += ': ' + sorgu;
						...
						window.document.title = window.document.originalTitle + pageStr;
						window.StateBeingProcessed = 0;
					}
					DataTableAttr[e.tableId] = e.rowAttributes;
					DataTableTdAttr[tableId] = e.tdsAttributes;
					DataTableSummary[e.tableId] = e.summary;
					DataTableCurrPage = e.CurrPage;
					...
					if (customRender && typeof window[customRender] == 'function')
					{
						...
						Page.CloseProcessingMessage();
						var glf = dts[e.tableId].GridLoadFunc;
						if(glf  && typeof window[glf] == "function")
							window[glf]();
						return 1;
					}
	...
	if (Page.GetParameter('iDisplayStart') > 0)
		options.iDisplayStart = Page.GetParameter('iDisplayStart');
	...
	if (dt.QueryParamsToUrl)
		window.onpopstate = function(event) {
			if (event.state && typeof event.state.sorgu != 'undefined')
			{
				var obj = event.state;
				window.StateBeingProcessed = 1;
				var tblId = Object.keys(dts)[0];
				var dataTable = $('#' + tblId).dataTable();
				var pageSize = dataTable.fnSettings()._iDisplayLength;
				$('INPUT[name=sorgu]').val(obj.sorgu);
				dataTable.fnPageChange(obj.start / pageSize);
			}
		};
	...
	toolBar.addClass('d-flex flex-row-reverse justify-content-between w-100');
	var buttons = [
		{text: 'Sıfırla', cb: 'TableStateClear', ftype: '', icon: 'ui-icon-refresh'},
	...
	if(dt.ShowToolBar)
		for(var i=0; i<buttons.length; i++)
		{
			...
			btn.click(function(){ToolbarItemClicked(this);});
		}
...
{
	var settings = $('#' + tableId).dataTable().fnSettings();
	var cols = settings.aoColumns;
	var customRender = dts[tableId].CustomRenderFunc;
	var vColumns = [];
	for(var i=0; i<cols.length; i++)
		if (cols[i].bVisible || typeof window[customRender] == "function")
			vColumns.push(cols[i].sName);
	return vColumns;
}

function ProcessCustomRenderParams(data)
{
	if (customRenderParams == null)
		return data;
	var tableId = dtTableIds[0];
	var vColumns = GetVisibleColumns(tableId);
...
function DataGridDelete(rowId, row, model)
{
	var tempCls = USE_BS_UI ? 'danger' : 'ui-state-highlight';
	if (typeof model != "string" || !model)
		model = $(row).parents('TABLE').first().attr('id');
	$(row).parent().children().removeClass(tempCls);
	$(row).addClass(tempCls);
	PageDeleteRecord(rowId, 'DataGridDelete', '', {id: rowId, model: model});
}

function ToolbarItemClicked(obj)
{
	var thisObj = this;
	if(typeof obj != "undefined")
		thisObj = obj;
	this.blur();
	var tableId  = $(thisObj).attr('table_id');
	var oTable = $('#' + tableId ).dataTable();
	var cb = $(thisObj).attr('cb');
	if (cb)
		window[cb](thisObj, tableId, oTable);
}
...
function AlanSecici()
{
	...
	for(var i = 0; i < inpts.length; i++)
	{
		...
		var vis = inpts.eq(i).prop('checked');
...
function Converter(obj, tblId, oTable)
{
	...
	if(typeof currPageExport == "undefined")
		currPageExport = 0;
	if (currPageExport == 0 && count > 1000 && !confirm(
		...
		return false;
	var fileType = String(obj);
	if (typeof obj == "object")
		fileType = $(obj).attr('ftype');
	...
	if (typeof Ara == "function")
	{
		...
		if( typeof GetCustomParams == "function")
			customParams = GetCustomParams();
		var searchParams = Ara(null, customParams, 1);
		for(var i in searchParams)
			params.push({name: i, value: searchParams[i]});
		params = ProcessCustomRenderParams(params);
	}
	var visibleCols = [];
	var expCols = ['Sec', 'islem'];
	for(var i=0; i<dt.aoColumns.length; i++)
		if (dt.aoColumns[i].bVisible && expCols.indexOf(dt.aoColumns[i].sName) < 0)
			visibleCols.push(dt.aoColumns[i].sName);
	params.push({ name: 'sConditionOperators', value: Ara(null, null, 2)});
	var colSize = [];
	var tbl = $('#' + tblId);
	var tblIsVisible = tbl.is(':visible');
	...
	if (total < 100 && cells.length > 0)
		colSize[0] += 100 - total;
	if (! tblIsVisible)
		colSize = [];
	var href = window.location.href.replace(/#.*$/, '');

	if (UseRewriteAct != 1 && ! href.match(/[?]/))
		href += '?';

	href += '&table_id=' + tblId + '&grid=1';
	...
	if (fileType == 'pdf' && ($.browser.chrome || count <= 999))
	{
		...
		$.post(href + '&onlyData=1', params, function(data){
			...
			if (! resp)
				...
				$.getScript(js[0])
					.done(function(){
						$.getScript(js[1]).done(function(){
							if (! tblIsVisible && dt.aoColumns.length > 8)
							{
								visibleCols = visibleCols.slice(0, 8);
	...
	else
	{
		...
		if (f.length == 0)
			...
			f = $(f);
		f.html('');
		var link = href + '&export=' + fileType;
		console.log(params);
		if(currPageExport != 0)
			link = link + '&currPageExport=' + DataTableCurrPage;
		f.attr('action', link);
		f.attr('method', 'POST');
...
function Bootstrapize(tableId)
{
	...
	$('<div class="panel-body no-padding">').append(table).appendTo(panel);
	toolbars.eq(1).addClass('panel-footer clearfix').appendTo(panel);
}

function Bootstrapize4(tableId)
{
	var table = $('#' + tableId).addClass('table table-striped table-hover').removeAttr('border');
	var panel = table.parents('.dataTables_wrapper').addClass('bsui-table-wrapper card');
	var toolbars = panel.find('.fg-toolbar').attr('class', '');
	toolbars.eq(0).addClass('card-header clearfix').appendTo(panel);
	$('<div class="card-body no-padding">').append(table).appendTo(panel);
	toolbars.eq(1).addClass('card-footer clearfix').appendTo(panel);
}
	...
	$.fn.dataTableExt.oPagination.ersoft_paging = {
		"fnInit": function ( oSettings, nPaging, fnCallbackDraw )
		{
			...
			for(var i=0; i<buttons.length; i++)
			{
				...
				if (cmd != 'page')
					btn.click(function(){
						var cmd = $(this).attr('cmd').split(' ')[0];
						oSettings.oApi._fnPageChange( oSettings, cmd);
						fnCallbackDraw( oSettings );
					});
			}
			Jui.InitButtons(nPaging, 1);
			var page = $('<input type="text" class="page_number normal-input ui-controlgroup-item" var_type="int">');
			var pageCnt = $('<input type="text" readOnly="readOnly" class="page_count normal-input ui-controlgroup-item hidden-xs">');
